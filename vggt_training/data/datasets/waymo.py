...
class WaymoDataset(BaseDataset):
    def __init__(
        ...
        common_conf,
        split: str = "train",
        Waymo_DIR: str = "/home/stud/xiji/repos/CoProU-VO-MF/storage/waymo_original_size",
        ...
        self.Waymo_DIR = (osp.join(Waymo_DIR, 'waymo_original_size_train') if self.training 
                          else osp.join(Waymo_DIR, 'waymo_original_size_val'))
        self.Waymo_Split = ('vggt_training/data/datasets/waymo_day_light_split.txt' if self.training 
                          else 'vggt_training/data/datasets/waymo_day_light_split_val.txt')
        ...
        txt_path = osp.join(self.Waymo_DIR, "sequence_list.txt")
        if osp.exists(txt_path):
            with open(txt_path, 'r') as f:
                sequence_list = [line.strip() for line in f.readlines()]
        else:
            # Generate sequence list and save to txt            
            sequence_list = glob.glob(osp.join(self.Waymo_DIR, "*"))            
            sequence_list = [file_path.split(self.Waymo_DIR)[-1].lstrip('/') for file_path in sequence_list]
        ...
        with open(self.Waymo_Split, 'r') as f:
            day_light_sequences = [line.strip() for line in f if line.strip()]

        self.sequence_list = [seq for seq in sequence_list if seq in day_light_sequences]
        self.sequence_list_len = len(self.sequence_list)
        
         # --- Precompute sorted .jpg files per sequence ---
        self.seq_to_images = {}
        for seq_name in self.sequence_list:
            seq_dir = osp.join(self.Waymo_DIR, seq_name, "images")
    ...
    def get_data(
        ...
        try:
            camera_parameters = np.loadtxt(
                osp.join(self.Waymo_DIR, seq_name, "poses.txt"), 
                delimiter=" ", 
                skiprows=1
            )
            intrinsic = np.loadtxt(
                osp.join(self.Waymo_DIR, seq_name, "intrinsics.txt"), 
            )
