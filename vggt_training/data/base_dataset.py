...
class BaseDataset(Dataset):
    ...
    def get_sequence_ids(self, full_seq_num: int, seq_len: int, expand_rate: int=1):
        """
        Randomly sample a contiguous subsequence of length `seq_len`
        from a full sequence of length `full_seq_num`, centered around
        a random anchor index. 
        """
        stride = np.random.randint(1, expand_rate + 1)
        seq_len = seq_len * stride
        if full_seq_num <= 0 or seq_len <= 0:
            raise ValueError("Both full_seq_num and seq_len must be positive.")
        if seq_len > full_seq_num:
            raise ValueError("seq_len cannot exceed full_seq_num.")

        half_left = int(np.floor(seq_len / 2))
        half_right = int(np.ceil(seq_len / 2))

        # valid anchor range so window fits fully in [0, full_seq_num)
        low = half_left
        high = full_seq_num - half_right + 1  # +1 because randint upper bound is exclusive

        anchor_id = np.random.randint(low, high)

        start = anchor_id - half_left
        end = anchor_id + half_right

        assert end - start == seq_len, f"length mismatch: {end - start} != {seq_len}"
        assert 0 <= start and end <= full_seq_num, f"out of range: {start}, {end}, {full_seq_num}"
