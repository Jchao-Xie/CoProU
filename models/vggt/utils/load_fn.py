...
def load_and_preprocess_images(image_path_list, mode="crop", target_size=518, patch_size=14, rescale_factor=1, intrinsic=None):
    """
    ...
    if rescale_factor < 1:
        raise ValueError("we only zoom in (rescale_factor > 1) to preserve more details, zoom out is not supported")
    
    if rescale_factor != 1 and intrinsic is None:
        raise ValueError("Intrinsic parameters must be provided when rescaling images")
    ...
    for image_path in image_path_list:
        ...
        if rescale_factor != 1:
            ...
            crop_w = max(2, int(round(width / rescale_factor)))
            crop_h = max(2, int(round(height / rescale_factor)))

            sx = width / crop_w
            sy = height / crop_h

            # Choose crop origin so that principal point stays fixed after resize:
            # cx' = (cx - x0) * sx = cx  =>  x0 = cx - cx / sx
            # cy' = (cy - y0) * sy = cy  =>  y0 = cy - cy / sy
            x0 = int(round(cx - cx / sx))
            y0 = int(round(cy - cy / sy))

            x0 = max(0, min(x0, width - crop_w))
            y0 = max(0, min(y0, height - crop_h))

            img_before = img.copy()  # Keep a copy of the original image for debugging
