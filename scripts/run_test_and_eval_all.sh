...
CHECKPOINT_DIR="/home/stud/xiji/SC-Depth_Anything/checkpoints/single_unty_nusc_6Hz/06-03-12:58"
...
find "$CHECKPOINT_DIR" -name "exp_pose_checkpoint_*.pth.tar" | sort -V | while read -r ckpt_path; do
    if [ ! -f "$ckpt_path" ]; then
    ...
    python "$EVAL_SCRIPT" --result "$result_dir" --align '7dof'
