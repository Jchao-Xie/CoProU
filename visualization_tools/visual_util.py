...
def predictions_to_glb(
    ...
    filter_z_percentile=None,
    filter_z_percentile_per_img=None,
) -> trimesh.Scene:
    """
    ...
    if filter_z_percentile_per_img is not None:
        upper_percentile_z_per_img = np.percentile(
                                            pred_world_points[..., 2],
                                            filter_z_percentile_per_img,
                                            axis=(1, 2)
                                        )
        percentile_mask_per_img = pred_world_points[..., 2] <= upper_percentile_z_per_img[:, None, None]  
        percentile_mask_per_img = percentile_mask_per_img[..., None]
        percentile_mask_per_img_global = percentile_mask_per_img.reshape(-1)
        conf_mask = conf_mask & percentile_mask_per_img_global
    ...
    # Filter point far away, e.g. sky
    if filter_z_percentile is not None:
        upper_percentile_z = np.percentile(vertices_3d[:, 2], filter_z_percentile)
        percentile_mask = vertices_3d[:, 2] <= upper_percentile_z
        vertices_3d = vertices_3d[percentile_mask]
        if colors_rgb is not None:
            colors_rgb = colors_rgb[percentile_mask]
...
def integrate_camera_into_scene(scene: trimesh.Scene, transform: np.ndarray, face_colors: tuple, scene_scale: float):
    ...
    cam_width = scene_scale * 0.05 / 10
    cam_height = scene_scale * 0.1 / 10
