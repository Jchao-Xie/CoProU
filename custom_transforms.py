...
class Resizing(object):
    ...
    def __call__(self, images, intrinsics):
        ...
        in_h, in_w, _ = images[0].shape
        scaled_h, scaled_w = int(self.target_size[0]), int(self.target_size[1])
        x_scaling = scaled_w / in_w
        y_scaling = scaled_h / in_h

        output_intrinsics[0] *= x_scaling
        output_intrinsics[1] *= y_scaling
        resized_images = [np.array(Image.fromarray(im.astype(np.uint8)).resize((scaled_w, scaled_h), resample=Image.LANCZOS)).astype(np.float32) for im in images]
