...
def tensor2array(tensor, max_value=None, colormap='rainbow', mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    ...
    if tensor.ndimension() == 2 or tensor.size(0) == 1:
        ...
        array = tensor.clone().detach().to(torch.float32)
...
def unnormalize_and_save(image, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], name="unnormalized_image.png"):
    """
    ...
    """
    unnormalized_image = image.clone().detach().to(torch.float32)
...
def depth_visualization(depth1, depth2, diff_depth, name):
    ...
    depth1 = depth1.clone().detach().to(torch.float32)
    depth2 = depth2.clone().detach().to(torch.float32)
    diff_depth = diff_depth.clone().detach().to(torch.float32)
...
def mask_visualization(mask, name):
    ...
    mask = mask.squeeze(0).cpu().to(torch.float32).numpy()
...
def heapmap_visualization(heatmap, name, vmin=None, vmax=None):
    heatmap = heatmap.clone().detach().to(torch.float32)
    
    plt.figure(figsize=(15, 5))
    plt.imshow(heatmap.squeeze(0).cpu().numpy(), cmap='viridis', vmin=0, vmax=1)
